#!/usr/bin/env python3
"""m2_arms_local.py (n3 M2/M1 agent, Sat 19 Sep): two-pass structured selection + island self-consistency
arms vs production (g015), interleaved per conversation. Derived from gap_arms_local.py:
...
ap.add_argument('--arms', default='g015,tp1,tp1x,tp2,sc5')
a = ap.parse_args()
...
# n3 M2/M1 arms (Sat 19 Sep): name: (unit_mode, gap, extra env, pipeline CFG overrides). g015 = production.
CFG_DEFAULTS = {'two_pass': 0, 'tp_examples': 1, 'tp_max_cands': 8, 'tp_min_cands': 2, 'tp_extent': 'pick', 'sc_n': 1, 'sc_t': 0.7}
ARMS = {
    'sentence': ('sentence', None, {}, {}),
    'g015': ('island', '0.15', {}, {}),
    'tp1': ('island', '0.15', {}, {'two_pass': 1}),                       # M2: candidates (structured) + selection
    'tp1x': ('island', '0.15', {}, {'two_pass': 1, 'tp_examples': 0}),    # M2 without the hard examples
    'tp2': ('island', '0.15', {}, {'two_pass': 2}),                       # selection over the cited clusters only
    'sc5': ('island', '0.15', {}, {'sc_n': 5, 'sc_t': 0.6}),              # M1: citation self-consistency, island path
    'tp1c': ('island', '0.15', {}, {'two_pass': 1, 'tp_extent': 'cluster'}),   # M2, location only (production extent)
    'tp2c': ('island', '0.15', {}, {'two_pass': 2, 'tp_extent': 'cluster'}),
}
...
def main():
    ...
    for i, fn in enumerate(names):
        ...
        for arm in arms:
            ...
            detail[arm][fn] = {'n_units': n_units[arm][fn], 'gap': ARMS[arm][1], 'unit_mode': ARMS[arm][0], 'cfg': ARMS[arm][3],
                               'qa_s': dt, 'res': res}
    ...
    for arm in arms:
        r = {'unit_mode': ARMS[arm][0], 'gap': ARMS[arm][1], 'cfg': ARMS[arm][3], 'per_conv': {}, 'cli': {},
             'latency_s': {'mean': sum(lat[arm]) / len(lat[arm]), 'max': max(lat[arm]),
                           'p95': sorted(lat[arm])[min(len(lat[arm]) - 1, int(round(0.95 * (len(lat[arm]) - 1))))]},
