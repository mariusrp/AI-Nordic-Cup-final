"""lin_sprint = lineage6 + SPRINT-LOCK RESERVE + FLEE ENERGY + RIPENESS PATIENCE (idea from public team JBrO910:
35-45% of victims could not sprint because energy < max_e/5 when they first saw the predator; fix: never breed below
the sprint lock + a reserve of ~120 from t=600, eat unripe fruit only when < 30% energy). All switches in LIN (sr_*):
  sr_birth: from sr_t0 a scheduled birth or a pre-senescence conversion must leave the parent >= max_e/5 + sr_res
            (fast agents exempt when sr_fast; flagged elders and emergency births exempt: they are dying / the herd is).
  sr_walk:  from sr_t0 the non-essential walks (wander for a tree, disperse to a second tree, tree approach when
            sr_walk >= 2) need energy >= max_e/5 + sr_res_w.
  sr_ripe:  every agent tracks fruit it saw being born (first seen at a spot it perceived empty on the previous step,
            dead-reckoned in its own frame); with energy >= sr_ripe_frac*max_e it leaves such a fruit until it is
            sr_ripe_age s old (energy 20 + 2*age, ripe 60 at 20 s) and holds its tree instead.
  Diagnostics (self.diag): late births below the lock+reserve, blocks, ripeness deferrals, danger-episode start energy.
...
LIN = dict(
    ...
    # lin_sprint (JBrO910 sprint-lock reserve + ripeness)
    sr_t0=600.0, sr_birth=0, sr_res=120.0, sr_pre=1, sr_fast=1,
    sr_walk=0, sr_res_w=120.0,
    sr_ripe=0, sr_ripe_frac=0.3, sr_ripe_age=20.0, sr_fr_tol=6.0, sr_fr_keep=300,
)
...
class Hivemind(bb_juke.Hivemind):
    def __init__(self, params=None):
        ...
        self.diag.update({"births": 0, "elder_births": 0, "elder_flag": 0, "wander_steps": 0, "wait_steps": 0,
                          ...
                          "late_births": 0, "late_below": 0, "late_below_sched": 0, "late_below_pre": 0, "late_below_elder": 0,
                          "late_below_emerg": 0, "sr_block_sched": 0, "sr_block_pre": 0, "sr_block_walk": 0, "sr_walk_below_px": 0.0,
                          "fr_new": 0, "fr_take": 0, "fr_take_young": 0, "fr_take_young_rich": 0, "ripe_defer": 0,
                          "dz_start": 0, "dz_start_locked": 0, "dz_start_below_res": 0})
    ...
    def act(self, agents, n_total=None):
        ...
        def sr_ok(a):
            """JBrO910 sprint-lock reserve: a late birth must leave the parent above max_e/5 + sr_res."""
            if not late or (p["sr_fast"] and a["speed"] >= p["fast_speed"]):
                return True
            return a["energy"] - 100.0 >= a["max_energy"] / 5 + p["sr_res"]
        pre = []
        for a in agents:
            if self.mem[a["agent_id"]].get("elder") or a["age"] < p["pre_age"] or a["energy"] <= p["pre_bar"]:
                continue
            if p["sr_birth"] and p["sr_pre"] and not sr_ok(a):
                self.diag["sr_block_pre"] += 1; continue
            pre.append(a)
        pre.sort(key=lambda a: (-(a["agent_id"] in elite), -a["energy"]))
        for a in pre:
            aid = a["agent_id"]
            lim = tgt + p["elder_over"] if aid in elite else tgt
        ...
        for a in agents:
            ...
            if a["speed"] < p["fast_speed"] and left < max_e / 5 + p["lock_margin"]:
                self.diag["res_block"] += 1; continue
            if p["sr_birth"] and not sr_ok(a):
                self.diag["sr_block_sched"] += 1; continue
        ...
        for a in agents:
            ...
            if act["spawn_agent"]:
                ...
                if a["energy"] > 100:
                    m["spawned_at"] = self.step
                if late and a["energy"] > 100:
                    self.diag["late_births"] += 1
                    kind = "emerg" if emerg else ("elder" if m.get("elder") else ("pre" if permits.get(aid) == "elder" else "sched"))
                    m["birth_kind"] = kind
                    if a["energy"] - 100.0 < a["max_energy"] / 5 + p["sr_res"] and a["speed"] < p["fast_speed"]:
                        self.diag["late_below"] += 1; self.diag["late_below_" + kind] += 1
    ...
    def _act_one(self, a, m, n, permit, stats, cur_by, fde):
        ...
        m["young"] = self._fruit_ages(a, m, fruits)
        ...
        # danger-episode start: energy vs the sprint lock (JBrO910 diagnostic)
        if danger is not None and not m.get("dz_on"):
            m["dz_e"] = energy; m["dz_t"] = self.step
            self.diag["dz_start"] += 1
            if energy < max_e / 5:
                self.diag["dz_start_locked"] += 1
            if energy < max_e / 5 + p["sr_res"]:
                self.diag["dz_start_below_res"] += 1
        m["dz_on"] = danger is not None
    ...
    def _forage(self, a, m, fruits, trees, sibs, edges, stats):
        ...
        young = m.get("young") or [False] * len(fruits)
        rich_ripe = energy >= p["sr_ripe_frac"] * max_e
        late = self.step * 0.1 >= p["sr_t0"]
        lockres_w = max_e / 5 + p["sr_res_w"]
        walk_ok = not (p["sr_walk"] and late and energy < lockres_w)
        ...
        if fruits and hungry:
            ...
            usable = []
            for i_f, f in enumerate(fruits):
                if f["distance"] > reach:
                    self.diag["reach_skip"] += 1
                    continue
                if p["sr_ripe"] and rich_ripe and young[i_f]:
                    self.diag["ripe_defer"] += 1
                    continue
        ...
        if target is not None:
            i_t = next(i for i, f in enumerate(fruits) if f is target)
            if target["distance"] <= speed + 5.0:
                self.diag["fr_take"] += 1
                if young[i_t]:
                    self.diag["fr_take_young"] += 1
                    if rich_ripe:
                        self.diag["fr_take_young_rich"] += 1
            m["wander"] = 0
            m["t"] = m.get("t", 0) + 1
        ...
        if st:
            ...
            if len(st) >= 2 and crowd >= p["disperse_sibs"] and not elder and energy > p["disperse_min_e"] \
                    and tree["distance"] < p["tree_stay"] + 15 and m.get("disp_cool", 0) <= 0 and walk_ok:
            ...
            elif tree["distance"] > p["tree_stay"] + 10 and not elder and (walk_ok or p["sr_walk"] < 2):
                move_dir = tree["angle"]; move_d = min(wspeed, tree["distance"] - p["tree_stay"])
                self.diag["w_tree"] += move_d
                if late and energy < lockres_w:
                    self.diag["sr_walk_below_px"] += move_d
            else:
            ...
            if rich and not elder and late and energy < lockres_w:
                self.diag["sr_walk_below_px"] += wspeed
                if not walk_ok:
                    self.diag["sr_block_walk"] += 1; rich = False
    ...
    def _fruit_ages(self, a, m, fruits):
        """Dead-reckoned fruit memory in the agent's own frame. Returns, per observed fruit, True when the fruit is
        known to be younger than sr_ripe_age (it was first seen at a spot this agent perceived empty on the previous
        step, so its age is known exactly). Unknown-age fruit counts as ripe."""
        p = self.p
        X, Y, H = m["X"], m["Y"], m["H"]
        hear = a["hearing_radius"]; vis = a["vision_range"]; half = a["vision_angle"] / 2.0
        mem = m.setdefault("fr", [])
        prev = m.get("fr_pose")
        tol = p["sr_fr_tol"]
        out = []
        seen = set()
        for f in fruits:
            wx = X + f["distance"] * math.cos(H + f["angle"]); wy = Y + f["distance"] * math.sin(H + f["angle"])
            best, bd = None, tol
            for j, e in enumerate(mem):
                dd = math.hypot(e[0] - wx, e[1] - wy)
                if dd < bd and j not in seen:
                    best, bd = j, dd
            if best is None:
                born = False
                if prev is not None:
                    px, py, ph, phear, pvis, phalf = prev
                    dd = math.hypot(wx - px, wy - py)
                    if dd < phear - 3.0:
                        born = True
                    elif dd < pvis - 5.0 and abs(wrap(math.atan2(wy - py, wx - px) - ph)) < phalf - 0.05:
                        born = True
                mem.append([wx, wy, self.step, self.step, born])
                best = len(mem) - 1
                if born:
                    self.diag["fr_new"] += 1
            e = mem[best]
            e[3] = self.step
            seen.add(best)
            out.append(bool(e[4]) and (self.step - e[2]) * 0.1 < p["sr_ripe_age"])
        # forget fruit that should be perceived now but is not (eaten) and stale entries
        keep = []
        for j, e in enumerate(mem):
            if j in seen:
                keep.append(e); continue
            dd = math.hypot(e[0] - X, e[1] - Y)
            if dd < hear - 3.0 or self.step - e[3] > p["sr_fr_keep"]:
                continue
            keep.append(e)
        m["fr"] = keep
        m["fr_pose"] = (X, Y, H, hear, vis, half)
        return out
