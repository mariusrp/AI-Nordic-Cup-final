"""lineage (Sat 19 Sep, LINEAGE ECONOMY lane): bb_juke's predator model + a demographic redesign.
...
LIN = dict(
    # breeding / lineage
    lottery_t=600.0, herd_max=12, herd_min=3, herd_div=3.0, herd_emerg=2, elite_margin=0.05, elite_min=2,
    fast_speed=15.1, w_speed=3.0, w_speed2=0.3, w_vis=1.0, w_hear=0.6, w_maxe=0.3, w_cone=0.2, w_sprint=0.2,
    birth_gap=6.0, young_gap=40.0, spawn_bar=200.0, spawn_bar_late=230.0, res0=60.0, res_slope=0.02, lock_margin=10.0,
    low_herd_any=3,   # if n < target - low_herd_any, the top-3 by fitness may breed even if not elite
    # senescence
    elder_from_age=55.0, elder_tol=0.25, elder_steps=2, elder_bar=101.0, elder_over=2, elder_reach=60.0,
    # economy
    eat_gap=25.0, reach_hi=0.6, reach_hi_d=90.0, reach_mid=0.35, reach_mid_d=160.0, walk_cap=10.0, back_cap=12.0,
    wander_frac=0.45, wander_min=140.0, disperse_sibs=3, disperse_d=60.0, tree_stay=35.0,
    scan_rate=0.08,
)
...
class Hivemind(bb_juke.Hivemind):
    ...
    def act(self, agents, n_total=None):
        ...
        for a in agents:
            ...
            if m.get("elder"):
                if e > p["elder_bar"] and n < tgt + p["elder_over"]:
                    permits[aid] = "elder"
                continue
            if aid not in elite or e <= max(bar, 101.0):
                continue
            left = e - 100.0
            if left < reserve:
    ...
    def _forage(self, a, m, fruits, trees, sibs, edges, stats):
        ...
        elif energy > p["reach_hi"] * max_e:
            reach = max(hear + 10.0, p["reach_hi_d"])
        elif energy > p["reach_mid"] * max_e:
            reach = p["reach_mid_d"]
        else:
            reach = 1e9
        target = None
        ...
        if st:
            tree = st[0]
            if len(st) >= 2 and crowd >= p["disperse_sibs"] and not elder and energy > 0.3 * max_e \
            ...
            elif tree["distance"] > p["tree_stay"] + 10:
        ...
        if close and move_d == 0.0:
            ...
            move_dir = wrap(s["angle"] + math.pi); move_d = min(wspeed, 3.0)
        m["t"] = m.get("t", 0) + 1
        return move_d, move_dir, turn
