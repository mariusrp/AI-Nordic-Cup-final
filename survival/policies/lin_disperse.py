"""lin_disperse = lineage6 + LATE DISPERSION + LATE AGE STAGGER + BARREN-TREE EXIT (surv2 designer lane, 19 Sep 2026 evening).
DEFAULTS = the best-measured arm S7 (stag_on=1 + barren_on=1, disp_on=0). Frozen scorer, paired vs lineage6 on seeds
5000-5031: S7 +62.3 se 50.0 (z 1.25, 17/32 wins); stag alone +50.4 se 39.2 (z 1.29); barren alone +31.5 se 39.4;
late dispersion alone -17.2 se 55.8 and dispersion+stag -42.0 se 58.6 (16 seeds). NONE is significant: byte-identical
lineage6 runs differ per seed across processes (null 16-seed paired se 53-70). LIN_PARAMS='{"stag_on":0,"barren_on":0}'
reproduces lineage6 behaviour.
  7. Late dispersion (disp_on; public team JBrO910's "one agent per tree" from t=600, their +42 over 96 games):
     from disp_t a non-elder agent treats a visible tree as OWNED when a non-elder sibling stands within own_d of
     it and closer to it than this agent; it holds its nearest free tree (within home_reach), walks to the nearest
     free visible tree when its own is owned, and when no free tree is in view and a non-elder sibling is within
     spread_d it hops away from the siblings for hop_steps (richer agent of the pair, energy > hop_min_e).
     Replaces the one-shot crowd rule (disperse_sibs/disperse_d) after disp_t.
  8. Late age stagger (stag_on; lineage v3 diagnostic: small late herds die of synchronized old age because the late
     spawn bar is unaffordable): from stag_t, whenever the youngest agent is older than stag_age s, the herd is at or
     below target and no other birth is planned this step, the best-fed agent (elite or not, elder or not) with
     energy > stag_bar spawns. Measured: fires only 0-4 times per game (the late herd is poor exactly when it is old
     and small; pre-senescence/elder conversion already makes ~90% of births).
  9. Barren-tree exit (barren_on; autopsy of lineage6's late starvation: starving agents spend their last 30 s
     standing at a tree with no fruit in view or waiting with no tree in view): a tree held for barren_s s with no
     fruit perceived within fz_d is blacklisted for black_s s (odometry frame) and the agent goes to another visible
     tree, or explores a straight xp_steps leg facing along it (xp_face) away from the barren tree; with no usable
     tree in view for wait_s s it explores (continuing its previous leg's direction, xp_persist) instead of standing.
...
LIN = dict(
    ...
    # late dispersion (7)
    disp_on=0, disp_t=600.0, own_d=90.0, home_reach=600.0, spread_d=300.0, hop_steps=20, hop_min_e=60.0, hop_on=1,
    # late age stagger (8)
    stag_on=1, stag_t=600.0, stag_age=35.0, stag_bar=160.0,
    # barren-tree exit + explore instead of waiting (9)
    barren_on=1, barren_t=600.0, barren_s=15.0, fz_d=120.0, black_s=60.0, black_r=40.0, explore_min_e=12.0,
    wait_s=10.0, xp_steps=25, xp_face=1, xp_persist=1,
)
...
class Hivemind(bb_juke.Hivemind):
    def __init__(self, params=None):
        ...
        self.diag.update({"births": 0, "elder_births": 0, "elder_flag": 0, "wander_steps": 0, "wait_steps": 0,
                          ...
                          "stag_births": 0, "own_leave": 0, "hops": 0, "w_hop": 0.0, "w_own": 0.0,
                          "barren": 0, "explore": 0, "w_xp": 0.0})
    ...
    def act(self, agents, n_total=None):
        ...
        self._last_e = {a["agent_id"]: a["energy"] for a in agents}
        alive = set()
        cur_by = {}
        fde = p["pred_mode"] == "fde"
        for a in agents:
        ...
        for f, e, aid in cands[:max(0, slots - min(n_el, slots))]:
            permits[aid] = "sched"
        if p["stag_on"] and t >= p["stag_t"] and youngest >= p["stag_age"] and n <= tgt and gap_ok and not permits:
            pool = [a for a in agents if a["energy"] > max(p["stag_bar"], 101.0)]
            if pool:
                permits[max(pool, key=lambda a: (a["energy"], -a["agent_id"]))["agent_id"]] = "stag"
        actions = []
        for a in agents:
            aid = a["agent_id"]
            m = self.mem[aid]
            ...
            if act["spawn_agent"]:
                ...
                self.diag["births"] += 1
                pk = permits.get(aid)
                self.diag["elder_births" if pk == "elder" else ("stag_births" if pk == "stag" else "sched_births")] += 1
    ...
    def _forage(self, a, m, fruits, trees, sibs, edges, stats):
        ...
        if p["barren_on"] and self.step * 0.1 >= p["barren_t"] and not elder:
            r, trees = self._barren(a, m, fruits, trees, sibs, edges)
            if r is not None:
                m["t"] = m.get("t", 0) + 1
                return r
        st = sorted(trees, key=lambda o: o["distance"]) if trees else []
        crowd = sum(1 for s in sibs if s["distance"] < p["disperse_d"])
        late = p["disp_on"] and self.step * 0.1 >= p["disp_t"] and not elder
        if late:
            r = self._late_hold(a, m, st, sibs, edges)
            if r is not None:
                m["t"] = m.get("t", 0) + 1
                return r
            st = []  # no tree to hold -> fall through to the no-tree branch (wait / wander)
        if st:
            tree = st[0]
...
def _late_hold_impl(self, a, m, st, sibs, edges):
    """Late dispersion: one agent per tree. Returns (move_d, move_dir, turn) or None when no tree is held."""
    p = self.p
    aid = a["agent_id"]; energy = a["energy"]
    wspeed = min(a["speed"], p["walk_cap"])
    young = [(s, to_xy(s["distance"], s["angle"])) for s in sibs if not self.mem.get(s["id"], {}).get("elder")]

    me = (energy, -aid)

    def owned(tr):
        # a young sibling holds this tree against me: I am approaching it, or we both sit at it and it is poorer
        tx, ty = to_xy(tr["distance"], tr["angle"])
        for s, (sx, sy) in young:
            ds = math.hypot(sx - tx, sy - ty)
            if ds < p["own_d"]:
                if tr["distance"] > p["own_d"] or (self._last_e.get(s["id"], 0.0), -s["id"]) < me:
                    return True
        return False
    free = [tr for tr in st if tr["distance"] <= p["home_reach"] and not owned(tr)]
    if free:
        m["hop"] = 0
        tree = free[0]
        if st and tree is not st[0]:
            self.diag["own_leave"] += 1
        if tree["distance"] > p["tree_stay"] + 10:
            md = min(wspeed, tree["distance"] - p["tree_stay"])
            self.diag["w_own"] += md
            return md, tree["angle"], 0.0
        return 0.0, 0.0, p["scan_rate"] * m["scan_dir"]
    # a hop in progress continues until a free tree shows up (direction kept in the odometry frame)
    if m.get("hop", 0) > 0 and energy > p["hop_min_e"]:
        m["hop"] -= 1
        d = self._avoid_edges(wrap(m["hop_dir"] - m["H"]), edges)
        m["hop_dir"] = m["H"] + d
        self.diag["w_hop"] += wspeed
        return wspeed, d, 0.0
    # no free tree in view: spread away from young siblings (evicted from an owned tree, or the richer of a pair)
    near = [(s, xy) for s, xy in young if s["distance"] < p["spread_d"]]
    evicted = bool(st)  # trees in view, all held by siblings
    if evicted:
        near = young
    if p["hop_on"] and near and energy > p["hop_min_e"] and (evicted or all(me > (self._last_e.get(s["id"], 0.0), -s["id"]) for s, _ in near)):
        vx = sum(-x / max(s["distance"], 1.0) ** 2 for s, (x, y) in near)
        vy = sum(-y / max(s["distance"], 1.0) ** 2 for s, (x, y) in near)
        d = self._avoid_edges(math.atan2(vy, vx), edges)
        m["hop"] = int(p["hop_steps"]) - 1
        m["hop_dir"] = m["H"] + d
        self.diag["hops"] += 1; self.diag["w_hop"] += wspeed
        return wspeed, d, 0.0
    if evicted:
        return 0.0, 0.0, p["scan_rate"] * m["scan_dir"]  # too poor to leave: stand and scan, never approach a held tree
    return None


Hivemind._late_hold = _late_hold_impl


def _barren_impl(self, a, m, fruits, trees, sibs, edges):
    """Barren-tree exit (9): a tree held for barren_s s with no fruit perceived within fz_d is blacklisted for black_s s
    (odometry frame); with no usable tree in view the agent explores (xp_steps leg) instead of standing: after leaving a
    barren tree at once, after wait_s s of waiting otherwise. Returns (action or None, usable trees)."""
    p = self.p
    step = self.step
    X, Y, H = m["X"], m["Y"], m["H"]
    energy = a["energy"]
    wspeed = min(a["speed"], p["walk_cap"])
    black = [b for b in m.get("black", []) if b[2] > step]
    m["black"] = black

    def wpos(o):
        return X + o["distance"] * math.cos(H + o["angle"]), Y + o["distance"] * math.sin(H + o["angle"])

    def is_black(o):
        wx, wy = wpos(o)
        return any(math.hypot(wx - bx, wy - by) < p["black_r"] for bx, by, _u in black)

    def leg(direction):
        # walk a straight leg and turn to face along it, so the vision cone sweeps the ground ahead
        d = self._avoid_edges(wrap(direction), edges)
        m["xp"] = int(p["xp_steps"]) - 1
        m["xp_dir"] = H + d
        self.diag["explore"] += 1; self.diag["w_xp"] += wspeed
        return (wspeed, d, d if p["xp_face"] else 0.0)
    ok = [o for o in trees if not is_black(o)] if black else list(trees)
    if ok:
        m["xp"] = 0
        m["nt0"] = step
        near = min(ok, key=lambda o: o["distance"])
        if near["distance"] <= p["tree_stay"] + 15:
            wx, wy = wpos(near)
            hw = m.get("hold_w")
            if hw is None or math.hypot(wx - hw[0], wy - hw[1]) > p["black_r"]:
                m["hold_since"] = step
            m["hold_w"] = (wx, wy)
            if step - max(m["hold_since"], m.get("fz", -10 ** 9)) > p["barren_s"] * 10 and energy > p["explore_min_e"]:
                black.append((wx, wy, step + p["black_s"] * 10))
                self.diag["barren"] += 1
                m["hold_w"] = None
                ok = [o for o in ok if o is not near]
                if not ok:
                    return leg(near["angle"] + math.pi), ok
        else:
            m["hold_w"] = None
        return None, ok
    m["hold_w"] = None
    # no usable tree in view
    if m.get("xp", 0) > 0 and energy > p["explore_min_e"]:
        m["xp"] -= 1
        d = self._avoid_edges(wrap(m["xp_dir"] - H), edges)
        m["xp_dir"] = H + d
        self.diag["w_xp"] += wspeed
        return (wspeed, d, d if p["xp_face"] else 0.0), ok
    nt0 = m.setdefault("nt0", step)
    if not fruits and step - nt0 > p["wait_s"] * 10 and energy > p["explore_min_e"]:
        m["nt0"] = step
        if p["xp_persist"] and "xp_dir" in m:
            # keep searching along the previous leg's world direction (straight-line search covers new ground)
            return leg(m["xp_dir"] - H + (0.5 if (step // 100) % 2 else -0.5)), ok
        if sibs:
            vx = sum(-math.cos(s["angle"]) / max(s["distance"], 1.0) for s in sibs)
            vy = sum(-math.sin(s["angle"]) / max(s["distance"], 1.0) for s in sibs)
            return leg(math.atan2(vy, vx)), ok
        return leg(0.0), ok
    return None, ok


Hivemind._barren = _barren_impl
