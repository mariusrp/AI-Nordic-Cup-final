...
class Hivemind:
    ...
    def _act_one(self, a, m, n, spawned, fits, med, stats, cur_by, fde):
            ...
            for P in plist:
                ...
                if not P["own"] and p["fuse"] == 2:
                    ...
                    if any(math.hypot(sx - P["x"], sy - P["y"]) < d and perceives(P["x"], P["y"], P["phi"], sx, sy, p)
                           for sx, sy in sib_xy):
                        continue
                    dangers.append((P["x"], P["y"], d)); continue
            ...
            if not dangers:
                ...
                for t in m["tracks"]:
                    if t["seen"] < self.step and t.get("threat") and t["rest"] < p["rest_steps"]:
                        ...
                        if dd < p["threat_d"]:
                            dangers.append((dd * math.cos(ang), dd * math.sin(ang), dd))
        ...
        if not fde:
            ...
            vx = vy = 0.0
            for (x, y, dd) in dangers:
                if dd < p["back_d"]:
                    w = 1.0 / max(dd, 1.0)
                    vx -= w * x / max(dd, 1e-6); vy -= w * y / max(dd, 1e-6)
            if d < close_d:
                flee = self._avoid_edges(math.atan2(vy, vx), edges)
                md = sprint if energy > max_e / 5 + 10 else speed
