#!/bin/bash
# (drone_arms_dir.sh: as drone_arms.sh, code directory from env SERVE_CODE)
...
SERVE_CODE=${SERVE_CODE:-/workspace/ft/code/drone}
BASE="CODE=$SERVE_CODE UPSTREAM=/workspace/upstream PORTS=9053,22 P=22 BANK=/workspace/drone_evolve_prod_g22/drone/bankAll.pt R11=/workspace/drone_weights/r11.pt PYPATH=/workspace/i4/pylib"
for spec in "$@"; do
  name=${spec%%|*}; vars=${spec#*|}
  echo "=== $(date +%H:%M) drone arm $name: $vars"
  up=$(NAC_MERGER=drone POD=gpu python3 infra/pod.py exec "$BASE ARM=$name $vars bash $SERVE_CODE/bb3/serve_ft5.sh" 300 | tail -1)
  echo "$up"
