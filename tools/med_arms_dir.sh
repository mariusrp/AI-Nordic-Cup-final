#!/bin/bash
# (med_arms_dir.sh: same as med_arms.sh, server directory from env MED_SRC)
...
for spec in "$@"; do
  ...
  NAC_MERGER=medical POD=gpu python3 infra/pod.py bg "med_arm_$name" "cd ${MED_SRC:-/workspace/med-attach/medical} && source env.sh && MED_PORT=9054 $env MED_REQ_LOG=/workspace/logs/med_requests_arm_$name.jsonl \$PY server.py" | tail -1
